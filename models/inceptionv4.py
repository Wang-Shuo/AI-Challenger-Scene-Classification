...
model_path = {
    'inceptionv4': '/home/wangshuo/experiment/compet/AIC/models/pretrained_models/inceptionv4-58153ba9.pth'
}
...
class inceptionv4_place80(nn.Module):

    def __init__(self, num_classes, pretrained=True):
        super(inceptionv4_place80, self).__init__()
        model = InceptionV4()
        if pretrained:
            model.load_state_dict(torch.load(model_path['inceptionv4']))
            
        model.features._modules['22'] = nn.AdaptiveAvgPool2d(1)
        num_ftrs = model.classif.in_features
        del model.classif
        model.classif = lambda x:x
        self.features = model
        self.classif = nn.Linear(num_ftrs, num_classes)
...
if __name__ == '__main__':
    model = inceptionv4_place80(num_classes=80)
    print(model)
